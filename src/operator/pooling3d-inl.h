...
#ifndef MXNET_OPERATOR_POOLING3D_INL_H_
#define MXNET_OPERATOR_POOLING3D_INL_H_
...
namespace pool_enum {
enum Pooling3dOpInputs {kData};
enum Pooling3dOpOutputs {kOut};
enum Pooling3dOpType {kMaxPooling, kAvgPooling, kSumPooling};
}  // namespace pool_enum

struct Pooling3dParam : public dmlc::Parameter<Pooling3dParam> {
  TShape kernel;
  TShape stride;
  TShape pad;
  int pool_type;
  DMLC_DECLARE_PARAMETER(Pooling3dParam) {
...
class Pooling3dOp : public Operator {
 public:
  explicit Pooling3dOp(Pooling3dParam p) {
    this->param_ = p;
    LOG(FATAL) << "3d Pooling implemented just for CUDNN ";
...
Operator* CreateOp(Pooling3dParam param);


#if DMLC_USE_CXX11
class Pooling3dProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    Pooling3dProp *prop_sym = new Pooling3dProp();
