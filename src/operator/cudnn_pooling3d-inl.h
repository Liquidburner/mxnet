...
#ifndef MXNET_OPERATOR_CUDNN_POOLING3D_INL_H_
#define MXNET_OPERATOR_CUDNN_POOLING3D_INL_H_
#include <algorithm>
#include <vector>
...
class CuDNNPooling3dOp : public Operator {
 public:
  explicit CuDNNPooling3dOp(Pooling3dParam p) {
    param_ = p;
    init_cudnn_ = false;
    // TODO(xxx): fp16
 ...
 private:
  inline void Init(mshadow::Stream<gpu> *s,
    ...
    if (!init_cudnn_) {
      ...
      int in_tensorDimA[5] = {int(data.shape_[0]),int(data.shape_[1]),int(data.shape_[2]),int(data.shape_[3]),int(data.shape_[4])};
      ...
      int out_tensorDimA[5] = 		{int(out.shape_[0]),int(out.shape_[1]),int(out.shape_[2]),int(out.shape_[3]),int(out.shape_[4])};
      ...
      int windowDimA[3]  = {int(param_.kernel[0]),int(param_.kernel[1]),int(param_.kernel[2])};
      int paddingA[3]  = {int(param_.pad[0]),int(param_.pad[1]),int(param_.pad[2])};
      int strideA[3]  = {int(param_.stride[0]),int(param_.stride[1]),int(param_.stride[2])};
  ...
  Pooling3dParam param_;
};  // class CuDNNPooling3dOp
}  // namespace op
}  // namespace mxnet
